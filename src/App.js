  ...
  return (
    <motion.div
      ...
      <Router>
        ...
        {breakpoint <= 640 ? <MenuIcon onClick={onClickButton} className='fixed top-3 left-3 z-20 cursor-pointer sm:z-0 sm:text-red-400' /> : ''}
        <AnimatePresence exitBeforeEnter>
          <Switch>
            <Route path='/' exact><Start /></Route>
            <Route component={Home} path='/home' />
            <Route path='/about'><About active={active} /></Route>
            <Route path='/history' ><History active={active}></History></Route>
          </Switch>
        </AnimatePresence>
      </Router>
    </motion.div>
